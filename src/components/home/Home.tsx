...
const newAlbums = [
  ...
  { title: 'Indigo', artist: 'Town Hall', seed: 'Indigo', color: 'from-teal-900/80 to-emerald-900', image: album2 },
  {
    ...
    artist: 'Exit View',
    seed: 'AboveTheSky',
    color: 'from-amber-200/80 to-yellow-300',
    image: album3,
  },
];
    ...
    <div className='space-y-8'>
      <section>
        ...
        <div className='grid grid-cols-3 gap-5'>
          {newAlbums.map(album => (
            <div key={album.title} className='group relative  overflow-hidden rounded-2xl bg-theme-dark-900'>
              <img src={album.image} alt={album.title} className='size-full object-cover opacity-80' />
      ...
      <section>
        ...
        <div className='relative overflow-hidden rounded-2xl bg-theme-dark-900'>
          <img src={album2} alt='Most Popular' className='h-48 w-full object-cover opacity-60' />
      ...
      <section>
        ...
        <div className='grid grid-cols-2 gap-14 pl-6'>
          {liveConcerts.map(concert => (
            <div key={concert.seed} className='group relative h-65 rounded-2xl bg-theme-dark-900'>
              <div className='absolute inset-0 overflow-hidden rounded-2xl'>
                <img src={concert.image} alt='Live Concert' className='size-full object-cover opacity-70' />
              </div>
              ...
              <div className='absolute -left-6 bottom-5 w-90 flex flex-col gap-2 rounded-lg bg-theme-primary/90 px-8 py-3 '>
                ...
                <p className='text-xs text-white/90'>{concert.viewers} people watching right now</p>
              </div>
            </div>
          ))}
        </div>
